...
export type OnboardingAction =
  ...
  | { type: "GO_NEXT" }
  | { type: "GO_BACK" }
  | { type: "GO_TO"; step: StepId; returnTo?: StepId }
  | { type: "SAVE_START" }
  | { type: "SAVE_SUCCESS" }
...
export function onboardingReducer(
  ...
  switch (action.type) {
      ...
      if (step === "accountType" && field === "type") {
        if (value === "individual") {
          data = {
            ...
            officeDetails: initialFormData.officeDetails,
    ...
    case "GO_NEXT": {
      //const current = steps[dynamicPageName];

      const current = steps[state.currentStep];
      if (!current.isValid(state.data))
        return { ...state, attemptedAdvance: true };
      const next = state.returnTo ?? current.next(state.data);
      console.log({
        currentStep: next,
